...
std::vector<FriendshipRecord> FriendDAO::getFriends(std::uint64_t userId) const
{
    ...
    std::vector<FriendshipRecord> friendships;
    if (!result) {
        return friendships;
    }
    friendships.reserve(result->rows.size());
    for (const auto& row : result->rows) {
        friendships.push_back(FriendshipRecord{detail::integer<std::uint64_t>(row, "user_id"),
                                               detail::integer<std::uint64_t>(row, "friend_id"),
                                               detail::value(row, "relation_status"),
