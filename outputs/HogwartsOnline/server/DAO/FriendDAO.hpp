...
public:
    ...
    [[nodiscard]] bool createFriendship(const FriendshipRecord& f);
    [[nodiscard]] std::vector<FriendshipRecord> getFriends(std::uint64_t userId) const;
    [[nodiscard]] bool updateFriendshipStatus(std::uint64_t userId, std::uint64_t friendId, const std::string& status);
