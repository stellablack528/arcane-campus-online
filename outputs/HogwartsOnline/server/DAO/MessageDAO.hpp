...
public:
    ...
    [[nodiscard]] bool saveMessage(const MessageRecord& msg);
    [[nodiscard]] std::optional<MessageRecord> getMessageById(std::uint64_t id) const;
    [[nodiscard]] std::vector<MessageRecord> getRoomMessages(std::uint64_t roomId, std::uint32_t limit) const;
    [[nodiscard]] bool deleteMessage(std::uint64_t id);
private:
